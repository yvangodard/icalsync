...
module Base32
  TABLE = '1234567890abcdefghijklmnopqrstuv'.freeze
  ...
  class Chunk
    ...
    def decode
      raise 'This function needs to be rewritten because of strip 61 is "=" but now strip set to 0'
  ...
  def self.random_base32(length=16, padding=true)
    ...
    padding ? random.ljust((length / 8.0).ceil * 8, '0') : random
  end
